...
rt_int32_t pulse_encoder_get_count(struct rt_pulse_encoder_device *pulse_encoder)
{
    ...
    result = (rt_int32_t)((rt_int16_t)__HAL_TIM_GET_COUNTER(&stm32_device->tim_handler) + stm32_device->over_under_flowcount * (AUTO_RELOAD_VALUE+1));
...
void pulse_encoder_update_isr(struct stm32_pulse_encoder_device *device)
{
    ...
    if (__HAL_TIM_GET_FLAG(&device->tim_handler, TIM_FLAG_UPDATE) != RESET)
    {
        ...
        {
            if (__HAL_TIM_GET_COUNTER(&device->tim_handler) > (AUTO_RELOAD_VALUE/2))//ÏÂÒç¼ÆÊýÖµ±Ø´óÓÚ(AUTO_RELOAD_VALUE/2)
            {
        ...
        {
            if (__HAL_TIM_GET_COUNTER(&device->tim_handler) < (AUTO_RELOAD_VALUE/2))//ÉÏÒç¼ÆÊýÖµ±ØÐ¡ÓÚ(AUTO_RELOAD_VALUE/2)
            {
                device->over_under_flowcount++;
            }
        }
